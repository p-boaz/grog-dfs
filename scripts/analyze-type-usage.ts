...
const OUTPUT_FILE = path.resolve(
  __dirname,
  "../knowledge/type-usage-analysis.md"
);
...
function findObjectCreationsForInterface(
    ...
    if (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node)) {
      ...
      if (node.parameters) {
        for (const param of node.parameters) {
          if (
            param.type &&
            param.type.getText(sourceFile).includes(interfaceName)
          ) {
            foundUsage = true;
          }
        }
...
function analyzeCombinedUsage(interfaceName: string): TypeUsage | null {
  ...
  for (const filePath of codeFiles) {
    ...
    try {
      const [propertyAccesses, objectCreations] =
        findObjectCreationsForInterface(interfaceName, filePath);
...
function generateReport(analysisResults: TypeUsage[]): string {
  ...
  for (const result of analysisResults) {
    ...
    report += `Defined in: \`${path.relative(
      process.cwd(),
      typeInfo.filePath
    )}\`\n\n`;
...
async function main() {
  ...
  console.log(
    `Found ${analysisResults.length} interfaces with usage mismatches`
  );
