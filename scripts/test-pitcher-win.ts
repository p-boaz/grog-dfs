...
// Logging setup
const logFile = path.join(process.cwd(), 'logs/pitcher-win-test.log');
fs.writeFileSync(logFile, '--- Pitcher Win Test Log ---\n\n', { flag: 'w' });
...
async function runTests() {
  ...
  try {
    ...
    if (coleWinStats) {
      ...
      log(`Team win percentage: ${(coleWinStats.teamWinPct * 100).toFixed(1)}%`);
      log(`ERA: ${coleWinStats.era.toFixed(2)}`);
      log(`WHIP: ${coleWinStats.whip.toFixed(2)}`);
      log(`Avg innings per start: ${coleWinStats.avgInningsPerStart.toFixed(1)}`);
  ...
  try {
    ...
    if (yankeeOffense) {
      ...
      log(`Run support rating: ${yankeeOffense.runSupportRating.toFixed(1)}/10`);
  ...
  try {
    const scherzerWinProb = await calculatePitcherWinProbability(MAX_SCHERZER_ID, TEST_GAME_PK);
    log(`Result: ${scherzerWinProb ? 'Success' : 'Failed'}`);
    if (scherzerWinProb) {
      log(`Overall win probability: ${scherzerWinProb.overallWinProbability}%`);
      log(`Expected DFS points: ${scherzerWinProb.expectedDfsPoints.toFixed(2)}`);
      log(`Confidence: ${scherzerWinProb.confidence}/100`);
      log(`Pitcher quality: ${scherzerWinProb.pitcherFactors.pitcherQuality.toFixed(1)}/10`);
      log(`Team offense: ${scherzerWinProb.teamFactors.runSupport.toFixed(1)}/10`);
      log(`Bullpen strength: ${scherzerWinProb.teamFactors.bullpenStrength.toFixed(1)}/10`);
      log(`Home/Away factor: ${scherzerWinProb.gameFactors.homeAway} (1=home, -1=away)`);
  ...
  try {
    ...
    if (bieberWinStats) {
      ...
      log(`Win percentage: ${(bieberWinStats.winPercentage * 100).toFixed(1)}%`);
      log(`Team win percentage: ${(bieberWinStats.teamWinPct * 100).toFixed(1)}%`);
      log(`ERA: ${bieberWinStats.era.toFixed(2)}`);
      log(`WHIP: ${bieberWinStats.whip.toFixed(2)}`);
      log(`Avg innings per start: ${bieberWinStats.avgInningsPerStart.toFixed(1)}`);
  ...
  try {
    ...
    if (bravesOffense) {
      ...
      log(`Run support rating: ${bravesOffense.runSupportRating.toFixed(1)}/10`);
