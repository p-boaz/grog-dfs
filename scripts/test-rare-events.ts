...
// Logging setup
const logFile = path.join(process.cwd(), 'logs/rare-events-test.log');
...
async function runTests() {
  ...
  try {
    const scherzerRareEvents = await calculateRareEventPotential(MAX_SCHERZER_ID, TEST_GAME_PK);
    log(`Result: ${scherzerRareEvents ? 'Success' : 'Failed'}`);
    if (scherzerRareEvents) {
      log(`Complete Game probability: ${scherzerRareEvents.eventProbabilities.completeGame.toFixed(2)}%`);
      log(`Quality Start probability: ${scherzerRareEvents.eventProbabilities.qualityStart.toFixed(2)}%`);
      log(`Shutout probability: ${scherzerRareEvents.eventProbabilities.shutout.toFixed(2)}%`);
      log(`No-Hitter probability: ${scherzerRareEvents.eventProbabilities.noHitter.toFixed(2)}%`);
      log(`Perfect Game probability: ${scherzerRareEvents.eventProbabilities.perfectGame.toFixed(2)}%`);
      log(`Expected DFS points from rare events: ${scherzerRareEvents.expectedRareEventPoints.toFixed(2)}`);
      log(`Risk-Reward Rating: ${scherzerRareEvents.riskRewardRating.toFixed(1)}/10`);
  ...
  try {
    const coleRareEvents = await calculateRareEventPotential(GERRIT_COLE_ID, TEST_GAME_PK);
    log(`Result: ${coleRareEvents ? 'Success' : 'Failed'}`);
    if (coleRareEvents) {
      log(`Complete Game probability: ${coleRareEvents.eventProbabilities.completeGame.toFixed(2)}%`);
      log(`Quality Start probability: ${coleRareEvents.eventProbabilities.qualityStart.toFixed(2)}%`);
      log(`Shutout probability: ${coleRareEvents.eventProbabilities.shutout.toFixed(2)}%`);
      log(`No-Hitter probability: ${coleRareEvents.eventProbabilities.noHitter.toFixed(2)}%`);
      log(`Perfect Game probability: ${coleRareEvents.eventProbabilities.perfectGame.toFixed(2)}%`);
      log(`Expected DFS points from rare events: ${coleRareEvents.expectedRareEventPoints.toFixed(2)}`);
      log(`Risk-Reward Rating: ${coleRareEvents.riskRewardRating.toFixed(1)}/10`);
  ...
  try {
    const verlanderHistory = await analyzeHistoricalRareEvents(JUSTIN_VERLANDER_ID);
    log(`Result: ${verlanderHistory ? 'Success' : 'Failed'}`);
    if (verlanderHistory) {
      log(`Pitcher: ${verlanderHistory.pitcher.name} (${verlanderHistory.pitcher.team})`);
      log(`Career Games: ${verlanderHistory.careerStats.totalGames}`);
      log(`Career Complete Games: ${verlanderHistory.careerStats.completeGames}`);
      log(`Career Shutouts: ${verlanderHistory.careerStats.shutouts}`);
      log(`Career No-Hitters: ${verlanderHistory.careerStats.noHitters}`);
      log(`Career Completion Rate: ${(verlanderHistory.careerStats.completionRate * 100).toFixed(1)}%`);
      log(`Current Season Games: ${verlanderHistory.seasonStats.totalGames}`);
      log(`Season Complete Games: ${verlanderHistory.seasonStats.completeGames}`);
  ...
  try {
    ...
    if (bieberHistory) {
      log(`Pitcher: ${bieberHistory.pitcher.name} (${bieberHistory.pitcher.team})`);
      ...
      log(`Career Completion Rate: ${(bieberHistory.careerStats.completionRate * 100).toFixed(1)}%`);
  ...
  try {
    const kershawRareEvents = await calculateRareEventPotential(CLAYTON_KERSHAW_ID, TEST_GAME_PK);
    log(`Result: ${kershawRareEvents ? 'Success' : 'Failed'}`);
    if (kershawRareEvents) {
      log(`Complete Game probability: ${kershawRareEvents.eventProbabilities.completeGame.toFixed(2)}%`);
      log(`Quality Start probability: ${kershawRareEvents.eventProbabilities.qualityStart.toFixed(2)}%`);
      log(`Expected DFS points from rare events: ${kershawRareEvents.expectedRareEventPoints.toFixed(2)}`);
      log(`Risk-Reward Rating: ${kershawRareEvents.riskRewardRating.toFixed(1)}/10`);
