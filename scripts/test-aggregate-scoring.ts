...
import { calculatePitcherDfsProjection, rankPitcherProjections } from '../lib/mlb/dfs-analysis/shared/aggregate-scoring';

// Setup logging
const LOG_FILE = path.join(__dirname, '../logs/aggregate-scoring-test.log');
fs.writeFileSync(LOG_FILE, '--- Aggregate Scoring Module Test ---\n\n', 'utf-8');
...
async function testAggregateScoringModule() {
  try {
    ...
    validProjections.forEach(p => {
      ...
      log(`- Range: ${p.projection.points.floor.toFixed(1)} - ${p.projection.points.upside.toFixed(1)}`);
      log(`- Projected Innings: ${p.projection.stats.projectedInnings.toFixed(1)}`);
      log(`- Projected Strikeouts: ${p.projection.stats.projectedStrikeouts.toFixed(1)}`);
      log(`- Win Probability: ${p.projection.stats.winProbability.toFixed(1)}%`);
      log(`- Projected Earned Runs: ${p.projection.stats.projectedEarnedRuns.toFixed(1)}`);
      ...
      log(`- Strikeouts: ${p.projection.points.breakdown.strikeouts.toFixed(1)}`);
      log(`- Win: ${p.projection.points.breakdown.win.toFixed(1)}`);
      log(`- Rare Events: ${p.projection.points.breakdown.rareEvents.toFixed(1)}`);
    ...
    rankings.rankings.forEach(rank => {
      log(`${rank.rank}. ${rank.pitcher.name || `Pitcher ${rank.pitcher.id}`}: ${rank.points.toFixed(1)} points (Value: ${rank.value.toFixed(2)} pts/$1K)`);
    });
