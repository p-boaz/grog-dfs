 ...
 * 
 * Usage:
 * pnpm ts-node scripts/transform-types.ts
 */
...
function calculateSimilarity(interface1: TypeInfo, interface2: TypeInfo): number {
...
function findSimilarInterfaces(interfaces: TypeInfo[], similarityThreshold = 0.7): { groups: TypeInfo[][], orphans: TypeInfo[] } {
  ...
  const orphans = interfaces.filter(intf => 
    !groups.some(group => group.some(g => g.name === intf.name))
  );
...
function generateUnifiedInterface(group: TypeInfo[]): string {
  ...
  for (const intf of group) {
    result += ` * - ${intf.name} (${path.relative(process.cwd(), intf.filePath)})\n`;
  }
  result += ` */\n`;
...
function generateAdapters(group: TypeInfo[]): string {
  ...
  for (const intf of group) {
    ...
    // Add properties with validation
    for (const [propName] of intf.properties) {
      result += `    ${propName}: source.${propName} ?? ${getDefaultValue(intf.properties.get(propName) || 'any')},\n`;
    }
...
function getDefaultValue(type: string): string {
  ...
  if (type.includes('Record<') || type.includes('object') || type === 'any') return '{}';
  return 'undefined';
}
  ...
  for (let i = 0; i < interfaces.length; i++) {
    ...
    for (let j = i + 1; j < interfaces.length; j++) {
      ...
      if (similarity >= 0.3) {
        ...
        report += `| ${current.name} | ${other.name} | ${(similarity * 100).toFixed(1)}% | ${sharedCount} | ${totalProps} |\n`;
      }
    }
  }
...
async function main() {
  ...
  console.log(`Found ${groups.length} groups of similar interfaces (${orphans.length} orphans)`);
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    const outputPath = path.join(OUTPUT_DIR, `unified-${group[0].name.toLowerCase()}.ts`);
    fs.writeFileSync(outputPath, unifiedInterface + '\n\n' + adapters, 'utf-8');
    console.log(`Generated unified interface for ${group[0].name} (${group.length} members)`);
  }
  
  // Generate type guards
  console.log('Generating type guards...');
  const typeGuards = generateTypeGuards(interfaces);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'type-guards.ts'), typeGuards, 'utf-8');
  
  // Generate similarity report
  console.log('Generating similarity report...');
  const report = generateSimilarityReport(interfaces);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'similarity-report.md'), report, 'utf-8');
