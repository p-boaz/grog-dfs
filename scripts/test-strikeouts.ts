...
// Logging setup
const logFile = path.join(process.cwd(), 'logs/strikeouts-test.log');
...
async function runTests() {
  ...
  try {
    const coleStrikeouts = await getPitcherStrikeoutStats(GERRIT_COLE_ID, "Yankees");
    ...
    if (coleStrikeouts) {
      ...
      log(`Strikeout percentage: ${(coleStrikeouts.strikeoutPercentage * 100).toFixed(1)}%`);
      log(`Strikeout stuff rating: ${coleStrikeouts.strikeoutStuff.toFixed(1)}/10`);
  ...
  try {
    ...
    if (redSoxVulnerability) {
      ...
      log(`Strikeouts per game: ${redSoxVulnerability.strikeoutsPerGame.toFixed(2)}`);
      log(`Strikeout rate: ${(redSoxVulnerability.strikeoutRate * 100).toFixed(1)}%`);
      log(`Vulnerability rating: ${redSoxVulnerability.strikeoutVulnerability.toFixed(1)}/10`);
  ...
  try {
    const coleVsRedSox = await calculateExpectedStrikeouts(GERRIT_COLE_ID, RED_SOX_ID, TEST_GAME_ID);
    log(`Result: ${coleVsRedSox ? 'Success' : 'Failed'}`);
    if (coleVsRedSox) {
      log(`Expected strikeouts: ${coleVsRedSox.expectedStrikeouts.toFixed(1)}`);
      log(`Range: ${coleVsRedSox.lowRange.toFixed(1)} - ${coleVsRedSox.highRange.toFixed(1)}`);
      log(`Expected DFS points from Ks: ${coleVsRedSox.expectedDfsPoints.toFixed(1)}`);
  ...
  try {
    const scherzerStrikeouts = await getPitcherStrikeoutStats(MAX_SCHERZER_ID, "Mets");
    log(`Result: ${scherzerStrikeouts ? 'Success' : 'Failed'}`);
    if (scherzerStrikeouts) {
      log(`Pitcher name: ${scherzerStrikeouts.name}`);
      log(`Strikeout rate (K/9): ${scherzerStrikeouts.strikeoutRate.toFixed(2)}`);
      log(`Strikeout percentage: ${(scherzerStrikeouts.strikeoutPercentage * 100).toFixed(1)}%`);
      log(`Strikeout stuff rating: ${scherzerStrikeouts.strikeoutStuff.toFixed(1)}/10`);
  ...
  try {
    ...
    if (angelsVulnerability) {
      ...
      log(`Strikeouts per game: ${angelsVulnerability.strikeoutsPerGame.toFixed(2)}`);
      log(`Strikeout rate: ${(angelsVulnerability.strikeoutRate * 100).toFixed(1)}%`);
      log(`Vulnerability rating: ${angelsVulnerability.strikeoutVulnerability.toFixed(1)}/10`);
  ...
  try {
    const burnesVsAthletics = await calculateExpectedStrikeouts(CORBIN_BURNES_ID, ATHLETICS_ID);
    log(`Result: ${burnesVsAthletics ? 'Success' : 'Failed'}`);
    if (burnesVsAthletics) {
      log(`Expected strikeouts: ${burnesVsAthletics.expectedStrikeouts.toFixed(1)}`);
      log(`Range: ${burnesVsAthletics.lowRange.toFixed(1)} - ${burnesVsAthletics.highRange.toFixed(1)}`);
      log(`Expected DFS points from Ks: ${burnesVsAthletics.expectedDfsPoints.toFixed(1)}`);
