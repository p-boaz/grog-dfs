...
async function runTests() {
  ...
  try {
    ...
    if (coleInningsStats) {
      ...
      log(`Avg innings per start: ${coleInningsStats.avgInningsPerStart.toFixed(2)}`);
      log(`Durability rating: ${coleInningsStats.durabilityRating.toFixed(1)}/10`);
  ...
  try {
    ...
    if (yankeesHookTendencies) {
      ...
      log(`Quick hook rating: ${yankeesHookTendencies.quickHookRating.toFixed(1)}/10`);
      log(`Bullpen usage rating: ${yankeesHookTendencies.bullpenUsageRating.toFixed(1)}/10`);
      log(`Starter innings per game: ${yankeesHookTendencies.starterInningsPerGame.toFixed(1)}`);
  ...
  try {
    const coleExpectedInnings = await calculateExpectedInnings(GERRIT_COLE_ID, TEST_GAME_ID);
    log(`Result: ${coleExpectedInnings ? 'Success' : 'Failed'}`);
    if (coleExpectedInnings) {
      log(`Expected innings: ${coleExpectedInnings.expectedInnings.toFixed(1)}`);
      log(`Range: ${coleExpectedInnings.ranges.low.toFixed(1)} - ${coleExpectedInnings.ranges.high.toFixed(1)}`);
      log(`Expected DFS points: ${coleExpectedInnings.expectedDfsPoints.toFixed(1)}`);
  ...
  try {
    const scherzerCGPotential = await calculateCompleteGamePotential(MAX_SCHERZER_ID, TEST_GAME_ID);
    log(`Result: ${scherzerCGPotential ? 'Success' : 'Failed'}`);
    if (scherzerCGPotential) {
      log(`Expected rare event points: ${scherzerCGPotential.expectedRareEventPoints.toFixed(2)}`);
      log(`Complete game probability: ${scherzerCGPotential.eventProbabilities.completeGame.toFixed(1)}%`);
      log(`No-hitter probability: ${scherzerCGPotential.eventProbabilities.noHitter.toFixed(2)}%`);
      log(`Quality start probability: ${scherzerCGPotential.eventProbabilities.qualityStart.toFixed(1)}%`);
      log(`Risk/reward rating: ${scherzerCGPotential.riskRewardRating.toFixed(1)}/10`);
  ...
  try {
    ...
    if (wheelerInningsStats) {
      ...
      log(`Avg innings per start: ${wheelerInningsStats.avgInningsPerStart.toFixed(2)}`);
      log(`Durability rating: ${wheelerInningsStats.durabilityRating.toFixed(1)}/10`);
  ...
  try {
    const pitcherIds = [GERRIT_COLE_ID, MAX_SCHERZER_ID, CORBIN_BURNES_ID, SHANE_BIEBER_ID];
    ...
    log(`Successfully retrieved stats for ${validStats.length} out of ${pitcherIds.length} pitchers`);
